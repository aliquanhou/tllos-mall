...
const fetchList = async () => {
  loading.value = true
  try {
    const res = await request({ url: '/user/after-sale', method: 'get', params: { status: activeTab.value === 'all' ? '' : activeTab.value } })
    const list = res.data?.list || res.data || []
    afterSaleList.value = list.map(item => ({
      id: item.id,
      after_sale_no: item.after_sale_no || item.refund_no || '',
      order_id: item.order_id,
      order_no: item.order_no || '',
      status: item.status || 0,
      type: item.type || 1,
      refund_amount: item.refund_amount || item.amount || 0,
      reason: item.reason || '',
      description: item.description || '',
      created_at: item.created_at || item.create_time || '',
      goods_name: item.goods_name || item.product_name || '',
      goods_image: item.goods_image || item.product_image || '',
      goods_spec: item.goods_spec || item.spec_info || '',
      goods_price: item.goods_price || item.price || 0
    }))
  } catch (e) {
...
const cancelApply = async (item) => {
  try {
    ...
    try {
      await request({ url: `/refunds/${item.id}/cancel`, method: 'post' })
...
const submitApply = async () => {
  ...
  try {
    await request({ url: '/refunds', method: 'post', data: applyForm })
