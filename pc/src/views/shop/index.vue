<template>
  <div class="shop-page">
    <!-- 店铺头部 -->
    <PageHeader title="店铺首页" subtitle="精选店铺 品质保障" />
    <div class="shop-header">
      <div class="container">
        <div class="shop-info">
          <div class="shop-logo"><img loading="lazy" :src="shop.logo" :alt="shop.name" /></div>
      ...
      <div class="shop-content">
        ...
        <div class="shop-categories" v-if="activeTab === 'home'">
          <div class="category-item" v-for="cat in shopCategories" :key="cat.id" @click="activeTab = 'products'">
            <div class="cat-icon"><img loading="lazy" :src="cat.icon" :alt="cat.name" /></div>
            <span>{{ cat.name }}</span>
          </div>
        </div>
        <!-- 商品列表 -->
        <div class="product-grid">
          <div class="product-card" v-for="product in products" :key="product.id" @click="$router.push(`/product/${product.id}`)">
            <div class="product-image"><img loading="lazy" :src="product.main_image" :alt="product.name" /></div>
...
const shopId = ref(route.query.shop_id || route.query.merchant_id || route.params.id || 1)
const shop = ref({
  name: '店铺', logo: '', rating: '4.9', product_count: 0, fans: '0', open_years: 1
})
...
const fetchProducts = async () => {
  loading.value = true
  try {
    const res = await request({ url: '/products', method: 'get', params: { merchant_id: shopId.value, limit: 20 } })
    ...
    // 从第一个商品中提取店铺信息
    if (list.length > 0 && list[0].merchant_name) {
      shop.value.name = list[0].merchant_name
    }
  } catch (e) {
