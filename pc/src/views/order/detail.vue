...
const fetchOrder = async () => {
  try {
    ...
    if (order.value) {
      const logs = []
      if (order.value.created_at) logs.push({ time: order.value.created_at, action: '用户提交订单' })
      if (order.value.pay_time) logs.push({ time: order.value.pay_time, action: '用户支付成功' })
      if (order.value.ship_time) logs.push({ time: order.value.ship_time, action: '商家发货' })
      if (order.value.complete_time) logs.push({ time: order.value.complete_time, action: '用户确认收货' })
...
const handleAfterSale = () => { router.push('/after-sale') }
