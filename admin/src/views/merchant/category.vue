<template>
  <el-card shadow="never">
    <template #header><div class="card-header"><span>商家分类</span><el-button type="primary" @click="handleAdd">新增分类</el-button></div></template>
...
import { getShopCategoryList, createShopCategory, updateShopCategory, deleteShopCategory } from '@/api/shopCenter'
const list = ref([]); const dialogVisible = ref(false); const isEdit = ref(false)
const form = ref({ id:null, name:'', sort:0, status:1 })
const fetchList = async () => { const res = await getShopCategoryList(); list.value = res.data.list||[] }
...
const handleSubmit = async () => { if(!form.value.name){ElMessage.warning('请输入分类名称');return}; if(isEdit.value){await updateShopCategory(form.value.id,form.value);ElMessage.success('更新成功')}else{await createShopCategory(form.value);ElMessage.success('创建成功')}; dialogVisible.value=false; fetchList() }
const handleDelete = async row => { await ElMessageBox.confirm(`确定删除分类"${row.name}"？`,'提示',{type:'warning'}); await deleteShopCategory(row.id); ElMessage.success('删除成功'); fetchList() }
onMounted(fetchList)
</script>
