...
## 1. 页面概述
### 功能描述
系统全局配置，包括基础配置、支付配置、物流配置、订单设置、数据字典等。本页面负责订单设置的管理操作，支持数据的增删改查、搜索筛选和状态管理。
...
| 方法 | 路径 | 控制器方法 | 说明 | 权限标识 |
|------|------|-----------|------|----------|
| GET | /api/v1/admin/system/order | SystemController@index | 订单设置列表 | system:list |
| POST | /api/v1/admin/system/order | SystemController@store | 新增订单设置 | system:create |
| GET | /api/v1/admin/system/order/{id} | SystemController@show | 订单设置详情 | system:view |
| PUT | /api/v1/admin/system/order/{id} | SystemController@update | 编辑订单设置 | system:edit |
| DELETE | /api/v1/admin/system/order/{id} | SystemController@destroy | 删除订单设置 | system:delete |
