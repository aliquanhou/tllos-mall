...
## 1. 页面概述
### 功能描述
系统全局配置，包括基础配置、支付配置、物流配置、订单设置、数据字典等。本页面负责操作日志的管理操作，支持数据的增删改查、搜索筛选和状态管理。
...
| 方法 | 路径 | 控制器方法 | 说明 | 权限标识 |
|------|------|-----------|------|----------|
| GET | /api/v1/admin/system/log | SystemController@index | 操作日志列表 | system:list |
| POST | /api/v1/admin/system/log | SystemController@store | 新增操作日志 | system:create |
| GET | /api/v1/admin/system/log/{id} | SystemController@show | 操作日志详情 | system:view |
| PUT | /api/v1/admin/system/log/{id} | SystemController@update | 编辑操作日志 | system:edit |
| DELETE | /api/v1/admin/system/log/{id} | SystemController@destroy | 删除操作日志 | system:delete |
