...
## 1. 页面概述
### 功能描述
管理商城所有订单，包括订单列表、售后、退款等，核心状态机模块。本页面负责订单管理模块总览的管理操作，支持数据的增删改查、搜索筛选和状态管理。
...
| 方法 | 路径 | 控制器方法 | 说明 | 权限标识 |
|------|------|-----------|------|----------|
| GET | /api/v1/admin/order/_index | OrderController@index | 订单管理模块总览列表 | order:list |
| POST | /api/v1/admin/order/_index | OrderController@store | 新增订单管理模块总览 | order:create |
| GET | /api/v1/admin/order/_index/{id} | OrderController@show | 订单管理模块总览详情 | order:view |
| PUT | /api/v1/admin/order/_index/{id} | OrderController@update | 编辑订单管理模块总览 | order:edit |
| DELETE | /api/v1/admin/order/_index/{id} | OrderController@destroy | 删除订单管理模块总览 | order:delete |
