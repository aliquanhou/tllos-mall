...
## 1. 页面概述
### 功能描述
商城辅助应用，包括充值管理、素材管理、文章资讯、消息管理、客服设置等。本页面负责消息管理的管理操作，支持数据的增删改查、搜索筛选和状态管理。
...
| 方法 | 路径 | 控制器方法 | 说明 | 权限标识 |
|------|------|-----------|------|----------|
| GET | /api/v1/admin/application/notice | ApplicationController@index | 消息管理列表 | application:list |
| POST | /api/v1/admin/application/notice | ApplicationController@store | 新增消息管理 | application:create |
| GET | /api/v1/admin/application/notice/{id} | ApplicationController@show | 消息管理详情 | application:view |
| PUT | /api/v1/admin/application/notice/{id} | ApplicationController@update | 编辑消息管理 | application:edit |
| DELETE | /api/v1/admin/application/notice/{id} | ApplicationController@destroy | 删除消息管理 | application:delete |
