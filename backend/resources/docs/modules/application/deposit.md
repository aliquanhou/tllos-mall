...
## 1. 页面概述
### 功能描述
商城辅助应用，包括充值管理、素材管理、文章资讯、消息管理、客服设置等。本页面负责充值管理的管理操作，支持数据的增删改查、搜索筛选和状态管理。
...
| 方法 | 路径 | 控制器方法 | 说明 | 权限标识 |
|------|------|-----------|------|----------|
| GET | /api/v1/admin/application/deposit | ApplicationController@index | 充值管理列表 | application:list |
| POST | /api/v1/admin/application/deposit | ApplicationController@store | 新增充值管理 | application:create |
| GET | /api/v1/admin/application/deposit/{id} | ApplicationController@show | 充值管理详情 | application:view |
| PUT | /api/v1/admin/application/deposit/{id} | ApplicationController@update | 编辑充值管理 | application:edit |
| DELETE | /api/v1/admin/application/deposit/{id} | ApplicationController@destroy | 删除充值管理 | application:delete |
