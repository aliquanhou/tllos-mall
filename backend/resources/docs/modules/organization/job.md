...
## 1. 页面概述
### 功能描述
企业组织架构，包括部门管理、岗位管理等。本页面负责岗位管理的管理操作，支持数据的增删改查、搜索筛选和状态管理。
...
| 方法 | 路径 | 控制器方法 | 说明 | 权限标识 |
|------|------|-----------|------|----------|
| GET | /api/v1/admin/organization/job | OrganizationController@index | 岗位管理列表 | organization:list |
| POST | /api/v1/admin/organization/job | OrganizationController@store | 新增岗位管理 | organization:create |
| GET | /api/v1/admin/organization/job/{id} | OrganizationController@show | 岗位管理详情 | organization:view |
| PUT | /api/v1/admin/organization/job/{id} | OrganizationController@update | 编辑岗位管理 | organization:edit |
| DELETE | /api/v1/admin/organization/job/{id} | OrganizationController@destroy | 删除岗位管理 | organization:delete |
