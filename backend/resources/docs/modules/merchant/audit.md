...
## 1. 页面概述
### 功能描述
多商户商城核心，管理商家入驻、审核、信息、商品等。本页面负责入驻审核的管理操作，支持数据的增删改查、搜索筛选和状态管理。
...
| 方法 | 路径 | 控制器方法 | 说明 | 权限标识 |
|------|------|-----------|------|----------|
| GET | /api/v1/admin/merchant/audit | MerchantController@index | 入驻审核列表 | merchant:list |
| POST | /api/v1/admin/merchant/audit | MerchantController@store | 新增入驻审核 | merchant:create |
| GET | /api/v1/admin/merchant/audit/{id} | MerchantController@show | 入驻审核详情 | merchant:view |
| PUT | /api/v1/admin/merchant/audit/{id} | MerchantController@update | 编辑入驻审核 | merchant:edit |
| DELETE | /api/v1/admin/merchant/audit/{id} | MerchantController@destroy | 删除入驻审核 | merchant:delete |
