...
## 1. 页面概述
### 功能描述
RBAC权限体系，包括角色管理、菜单管理、管理员、部门等。本页面负责公众号菜单的管理操作，支持数据的增删改查、搜索筛选和状态管理。
...
| 方法 | 路径 | 控制器方法 | 说明 | 权限标识 |
|------|------|-----------|------|----------|
| GET | /api/v1/admin/permission/menu | PermissionController@index | 公众号菜单列表 | permission:list |
| POST | /api/v1/admin/permission/menu | PermissionController@store | 新增公众号菜单 | permission:create |
| GET | /api/v1/admin/permission/menu/{id} | PermissionController@show | 公众号菜单详情 | permission:view |
| PUT | /api/v1/admin/permission/menu/{id} | PermissionController@update | 编辑公众号菜单 | permission:edit |
| DELETE | /api/v1/admin/permission/menu/{id} | PermissionController@destroy | 删除公众号菜单 | permission:delete |
