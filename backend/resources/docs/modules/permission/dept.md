...
## 1. 页面概述
### 功能描述
RBAC权限体系，包括角色管理、菜单管理、管理员、部门等。本页面负责部门管理的管理操作，支持数据的增删改查、搜索筛选和状态管理。
...
| 方法 | 路径 | 控制器方法 | 说明 | 权限标识 |
|------|------|-----------|------|----------|
| GET | /api/v1/admin/permission/dept | PermissionController@index | 部门管理列表 | permission:list |
| POST | /api/v1/admin/permission/dept | PermissionController@store | 新增部门管理 | permission:create |
| GET | /api/v1/admin/permission/dept/{id} | PermissionController@show | 部门管理详情 | permission:view |
| PUT | /api/v1/admin/permission/dept/{id} | PermissionController@update | 编辑部门管理 | permission:edit |
| DELETE | /api/v1/admin/permission/dept/{id} | PermissionController@destroy | 删除部门管理 | permission:delete |
