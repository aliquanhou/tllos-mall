...
## 1. 页面概述
### 功能描述
分销体系核心，管理分销商、等级、订单、佣金、设置等。本页面负责分销管理模块总览的管理操作，支持数据的增删改查、搜索筛选和状态管理。
...
| 方法 | 路径 | 控制器方法 | 说明 | 权限标识 |
|------|------|-----------|------|----------|
| GET | /api/v1/admin/distribute/_index | DistributeController@index | 分销管理模块总览列表 | distribute:list |
| POST | /api/v1/admin/distribute/_index | DistributeController@store | 新增分销管理模块总览 | distribute:create |
| GET | /api/v1/admin/distribute/_index/{id} | DistributeController@show | 分销管理模块总览详情 | distribute:view |
| PUT | /api/v1/admin/distribute/_index/{id} | DistributeController@update | 编辑分销管理模块总览 | distribute:edit |
| DELETE | /api/v1/admin/distribute/_index/{id} | DistributeController@destroy | 删除分销管理模块总览 | distribute:delete |
