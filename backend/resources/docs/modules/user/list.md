...
## 1. 页面概述
### 功能描述
管理商城所有用户，包括用户列表、等级、标签、认证等。本页面负责列表的管理操作，支持数据的增删改查、搜索筛选和状态管理。
...
| 方法 | 路径 | 控制器方法 | 说明 | 权限标识 |
|------|------|-----------|------|----------|
| GET | /api/v1/admin/user/list | UserController@index | 列表列表 | user:list |
| POST | /api/v1/admin/user/list | UserController@store | 新增列表 | user:create |
| GET | /api/v1/admin/user/list/{id} | UserController@show | 列表详情 | user:view |
| PUT | /api/v1/admin/user/list/{id} | UserController@update | 编辑列表 | user:edit |
| DELETE | /api/v1/admin/user/list/{id} | UserController@destroy | 删除列表 | user:delete |
