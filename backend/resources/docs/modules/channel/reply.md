...
## 1. 页面概述
### 功能描述
多渠道配置，包括渠道配置、公众号菜单、自动回复等。本页面负责自动回复的管理操作，支持数据的增删改查、搜索筛选和状态管理。
...
| 方法 | 路径 | 控制器方法 | 说明 | 权限标识 |
|------|------|-----------|------|----------|
| GET | /api/v1/admin/channel/reply | ChannelController@index | 自动回复列表 | channel:list |
| POST | /api/v1/admin/channel/reply | ChannelController@store | 新增自动回复 | channel:create |
| GET | /api/v1/admin/channel/reply/{id} | ChannelController@show | 自动回复详情 | channel:view |
| PUT | /api/v1/admin/channel/reply/{id} | ChannelController@update | 编辑自动回复 | channel:edit |
| DELETE | /api/v1/admin/channel/reply/{id} | ChannelController@destroy | 删除自动回复 | channel:delete |
