...
## 1. 页面概述
### 功能描述
多渠道配置，包括渠道配置、公众号菜单、自动回复等。本页面负责渠道设置模块总览的管理操作，支持数据的增删改查、搜索筛选和状态管理。
...
| 方法 | 路径 | 控制器方法 | 说明 | 权限标识 |
|------|------|-----------|------|----------|
| GET | /api/v1/admin/channel/_index | ChannelController@index | 渠道设置模块总览列表 | channel:list |
| POST | /api/v1/admin/channel/_index | ChannelController@store | 新增渠道设置模块总览 | channel:create |
| GET | /api/v1/admin/channel/_index/{id} | ChannelController@show | 渠道设置模块总览详情 | channel:view |
| PUT | /api/v1/admin/channel/_index/{id} | ChannelController@update | 编辑渠道设置模块总览 | channel:edit |
| DELETE | /api/v1/admin/channel/_index/{id} | ChannelController@destroy | 删除渠道设置模块总览 | channel:delete |
