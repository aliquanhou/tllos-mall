...
## 1. 页面概述
### 功能描述
可视化页面装修，包括页面装修、模板管理、轮播图、导航图标等。本页面负责轮播图的管理操作，支持数据的增删改查、搜索筛选和状态管理。
...
| 方法 | 路径 | 控制器方法 | 说明 | 权限标识 |
|------|------|-----------|------|----------|
| GET | /api/v1/admin/decorate/banners | DecorateController@index | 轮播图列表 | decorate:list |
| POST | /api/v1/admin/decorate/banners | DecorateController@store | 新增轮播图 | decorate:create |
| GET | /api/v1/admin/decorate/banners/{id} | DecorateController@show | 轮播图详情 | decorate:view |
| PUT | /api/v1/admin/decorate/banners/{id} | DecorateController@update | 编辑轮播图 | decorate:edit |
| DELETE | /api/v1/admin/decorate/banners/{id} | DecorateController@destroy | 删除轮播图 | decorate:delete |
