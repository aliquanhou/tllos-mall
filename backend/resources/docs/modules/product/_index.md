...
## 1. 页面概述
### 功能描述
商城核心模块，管理所有商品信息，包括商品列表、分类、品牌、评价等。本页面负责商品管理模块总览的管理操作，支持数据的增删改查、搜索筛选和状态管理。
...
| 方法 | 路径 | 控制器方法 | 说明 | 权限标识 |
|------|------|-----------|------|----------|
| GET | /api/v1/admin/product/_index | ProductController@index | 商品管理模块总览列表 | product:list |
| POST | /api/v1/admin/product/_index | ProductController@store | 新增商品管理模块总览 | product:create |
| GET | /api/v1/admin/product/_index/{id} | ProductController@show | 商品管理模块总览详情 | product:view |
| PUT | /api/v1/admin/product/_index/{id} | ProductController@update | 编辑商品管理模块总览 | product:edit |
| DELETE | /api/v1/admin/product/_index/{id} | ProductController@destroy | 删除商品管理模块总览 | product:delete |
