...
## 1. 页面概述
### 功能描述
商城资金管理，包括订单收款、退款记录、提现管理、商家结算等。本页面负责商家结算的管理操作，支持数据的增删改查、搜索筛选和状态管理。
...
| 方法 | 路径 | 控制器方法 | 说明 | 权限标识 |
|------|------|-----------|------|----------|
| GET | /api/v1/admin/finance/settlement | FinanceController@index | 商家结算列表 | finance:list |
| POST | /api/v1/admin/finance/settlement | FinanceController@store | 新增商家结算 | finance:create |
| GET | /api/v1/admin/finance/settlement/{id} | FinanceController@show | 商家结算详情 | finance:view |
| PUT | /api/v1/admin/finance/settlement/{id} | FinanceController@update | 编辑商家结算 | finance:edit |
| DELETE | /api/v1/admin/finance/settlement/{id} | FinanceController@destroy | 删除商家结算 | finance:delete |
