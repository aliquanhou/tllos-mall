...
## 1. 页面概述
### 功能描述
商城资金管理，包括订单收款、退款记录、提现管理、商家结算等。本页面负责提现管理的管理操作，支持数据的增删改查、搜索筛选和状态管理。
...
| 方法 | 路径 | 控制器方法 | 说明 | 权限标识 |
|------|------|-----------|------|----------|
| GET | /api/v1/admin/finance/withdraw | FinanceController@index | 提现管理列表 | finance:list |
| POST | /api/v1/admin/finance/withdraw | FinanceController@store | 新增提现管理 | finance:create |
| GET | /api/v1/admin/finance/withdraw/{id} | FinanceController@show | 提现管理详情 | finance:view |
| PUT | /api/v1/admin/finance/withdraw/{id} | FinanceController@update | 编辑提现管理 | finance:edit |
| DELETE | /api/v1/admin/finance/withdraw/{id} | FinanceController@destroy | 删除提现管理 | finance:delete |
